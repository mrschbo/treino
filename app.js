...
function createExerciseCard(exercise, workoutIndex, weekIndex) {
  const card = document.createElement("div");
  card.className = "exercise-card";
  card.innerHTML = `
    <div class="card-header">
      <h3>${exercise.nome}</h3>
      <button class="delete-btn" data-week="${weekIndex}" data-workout="${workoutIndex}" data-id="${exercise.id}">✕</button>
    </div>

    <div class="card-content">
      <div class="field">
        <label>Tipo:</label>
        <input type="text" class="tipo-input" value="${exercise.tipo || ''}" />
      </div>

      <div class="field">
        <label>Séries:</label>
        <input type="text" class="series-input" value="${exercise.series || ''}" />
      </div>

      <div class="field">
        <label>Foco:</label>
        <input type="text" class="foco-input" value="${exercise.foco || ''}" />
      </div>
  ...
  card.querySelector(".delete-btn").addEventListener("click", () => {
    if (confirm("Remover este exercício?")) {
      card.remove();
      weeks[weekIndex].workouts[workoutIndex].exercises = weeks[weekIndex].workouts[workoutIndex].exercises.filter(
        e => e.id !== exercise.id
      );
    }
  });
  ...
  card.querySelectorAll("input, textarea").forEach(input => {
    input.addEventListener("input", () => {
      exercise.nome = card.querySelector("h3").textContent;
      exercise.tipo = card.querySelector(".tipo-input").value;
      exercise.series = card.querySelector(".series-input").value;
      exercise.foco = card.querySelector(".foco-input").value;
      exercise.tecnica = card.querySelector(".tecnica-input").value;
      exercise.explicacao = card.querySelector(".explicacao-input").value;
    });
  });
...
function renderApp() {
  ...
  weeks.forEach((week, weekIndex) => {
    ...
    week.workouts.forEach((workout, workoutIndex) => {
      ...
      workoutHeader.innerHTML = `<h2>${workout.name}</h2>`;
      workoutHeader.addEventListener("click", () => {
        workoutDiv.classList.toggle("collapsed");
      });
      ...
      workout.exercises.forEach(exercise => {
        workoutContent.appendChild(createExerciseCard(exercise, workoutIndex, weekIndex));
      });
      ...
      addBtn.addEventListener("click", () => {
        const newId = Math.max(...workout.exercises.map(e => e.id || 0), 0) + 1;
        const newExercise = {
          id: newId,
          nome: "Novo exercício",
          tipo: "Máquina",
        ...
        workoutContent.insertBefore(createExerciseCard(newExercise, workoutIndex, weekIndex), addBtn);
      });

      workoutContent.appendChild(addBtn);
      workoutDiv.appendChild(workoutHeader);
      workoutDiv.appendChild(workoutContent);
